...
@section('title', 'Modifier le campus - ESEBAT')
@section('page-title', 'Modifier le campus')

@section('content')
<div class="row">
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">Modifier : {{ $campus->name }}</div>
            <div class="card-body">
                <form action="{{ route('campuses.update', $campus) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code', $campus->code) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('order_responsible_user_id') is-invalid @enderror" id="order_responsible_user_id" name="order_responsible_user_id">
                            ...
                            @foreach ($usersForResponsible as $u)
                                <option value="{{ $u->id }}" {{ old('order_responsible_user_id', $campus->order_responsible_user_id) == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->email }})</option>
                            @endforeach
                        </select>
